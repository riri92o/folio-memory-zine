...
(() => {
  ...
  class FolioBook extends HTMLElement {
    ...
    project(x, y, z = 0) {
      // A flat sheet viewed head-on must stay rectangular. Depth still changes
      // the scale of curled segments, but page height no longer introduces an
      // artificial trapezoid on the closed cover.
              ...
              new CustomEvent('pagechange', {
                detail: {
                  ...
                  pages:
                    this.spread === -1
                      ? []
                      : [this.spread * 2 + 1, this.spread * 2 + 2].filter(
                          (n) => n <= (this.pageCount ?? this._pages.length),
                        ),
                  cover: this.spread === -1,
                },
                bubbles: true,
    ...
    update() {
      ...
      const total = this.pageCount ?? this._pages.length;
      const numbers = [this.spread * 2 + 1, this.spread * 2 + 2].filter(
        (n) => n <= total,
      );
